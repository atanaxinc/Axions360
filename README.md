...
└── Dymmo                      Financial infrastructure platform
    └── Payments · Settlement · Compliance
```
...
| Principle | Design |
|-----------|--------|
| Multi-Tenant | Native — enforced at every layer |
| Multi-Language | Database-native i18n architecture |
| Multi-Currency | Native — 4 currency roles |
| Multi-Accounting Framework | Designed for it — decoupled from the core |
| Offline-First | Core design — not an add-on |
| Activation-Driven Growth | Core design — no migration ever |
| Payment Provider Agnostic | Unified abstraction layer |
| Framework-Agnostic Contracts | Rules live in the database, not in code |
...
Axions360 is built on contracts, not conventions. Before contributing, read the architecture documents:

| Document | Purpose |
|----------|---------|
| [ATANAX_ARCHITECTURE.md](docs/ATANAX_ARCHITECTURE.md) | System design, stack decisions, evaluated technologies |
| [AXIONS_KERNEL.md](docs/AXIONS_KERNEL.md) | Core schema, entity contracts, module rules |
| [AXIONS_SCHEMA_RULES.md](docs/AXIONS_SCHEMA_RULES.md) | Data conventions, i18n, opscodes, migrations |
| [CONTRIBUTING.md](CONTRIBUTING.md) | Git flow, versioning strategy, contribution rules |
